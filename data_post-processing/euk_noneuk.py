"""
This script uses an assembly FASTA and per-tool contig lists (EukRep, Kraken2, Tiara) that
indicate which contigs are eukaryotic vs non-eukaryotic, this script computes:

- contig_id   : ID from the FASTA header (up to the first whitespace)
- length      : contig length in bases (computed from the FASTA)
- eukrep      : binary label for EukRep (True/False)
- kraken2     : binary label for Kraken2 (True/False)
- tiara       : binary label for Tiara (True/False)

Inputs
- Assembly FASTA (plain or gzipped).
- Tool label files for each of: EukRep, Kraken2, Tiara.
  Each tool’s input should identify which contigs are eukaryotic (and/or non-eukaryotic).

Output
- A TSV with columns:contig_id, length, eukrep, kraken2, tiara
  where each tool column is a binary value (True/False) indicating that tool assigned the contig as eukaryotic.
"""
#!/usr/bin/env python3
